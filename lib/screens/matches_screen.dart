...
class _MatchesScreenState extends State<MatchesScreen> {
  ...
  Future<void> _load() async {
    try {
      ...
      final res = await _client.from('matches')
      ...
      debugPrint('Eşleşme yükleme hatası: $e');
  ...
  Map<String,dynamic> _other(Map<String,dynamic> m) {
    final uid = _client.auth.currentUser?.id ?? '';
    final u1 = m['user1'] as Map<String,dynamic>?;
    final u2 = m['user2'] as Map<String,dynamic>?;
    return (m['user1_id'] == uid ? u2 : u1) ?? {};
  }
                  ...
                  itemBuilder: (_, i) {
                    ...
                    return ListTile(
                      ...
                      onTap: () => Navigator.push(context, MaterialPageRoute(
                          builder: (_) => ChatScreen(matchId: _matches[i]['id']?.toString() ?? '', otherUser: other))),
                    );
                  })));
}
