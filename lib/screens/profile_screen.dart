...
class _ProfileScreenState extends State<ProfileScreen> {
  ...
  Future<void> _load() async {
    try {
      ...
      final res = await _client.from('users').select('*, photos(*), user_interests(*), preferences(*)').eq('id', uid).maybeSingle();
  ...
  Future<void> _pickPhoto() async {
    ...
    try {
      ...
      if (mounted) ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text('Fotoğraf yüklenemedi: $e')));
    }
  }
  ...
  Widget build(BuildContext context) {
    ...
    final interests = (_profile?['user_interests'] as List? ?? [])
        .map((i) => i['interest']?.toString() ?? '').where((s) => s.isNotEmpty).toList();
      ...
      body: _loading
          ...
          : _profile == null
              ? Center(child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [
                  const Text('Profil bulunamadı', style: TextStyle(fontSize: 16, color: kTextSecondary)),
              ...
              : RefreshIndicator(color: kPrimary, onRefresh: _load,
                  child: SingleChildScrollView(physics: const AlwaysScrollableScrollPhysics(), child: Column(children: [
                    Container(color: Colors.white, padding: const EdgeInsets.fromLTRB(16, 24, 16, 20), child: Column(children: [
                      ...
                      Text(name.isNotEmpty ? name : 'İsimsiz', style: const TextStyle(fontSize: 20, fontWeight: FontWeight.w500, color: kTextPrimary)),
                      const SizedBox(height: 4),
                      Row(mainAxisAlignment: MainAxisAlignment.center, children: [
                        const Icon(Icons.location_on_outlined, size: 14, color: kTextSecondary),
                        const SizedBox(width: 3),
                        Text('${city.isNotEmpty ? city : 'Şehir yok'}  •  $age yaş', style: const TextStyle(fontSize: 13, color: kTextSecondary)),
                      ]),
                      if (bio.isNotEmpty) ...[const SizedBox(height: 10),
                        Text(bio, textAlign: TextAlign.center, style: const TextStyle(fontSize: 13, color: kTextSecondary, height: 1.4))],
                    ])),
